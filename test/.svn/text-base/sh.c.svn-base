 ...
 * license       BSD
 *
 * file          /home/mycure/bpt/test/sh.c
 *
 * created       julien quintard   [thu jul 12 17:28:12 2007]
 * updated       julien quintard   [sat jul 14 05:17:28 2007]
 */
...
void			sh_load(t_bpt(sh)		*bpt,
				t_bpt_imm(sh)		*node,
				t_bpt_node(sh)		addr);

void			sh_unload(t_bpt(sh)		*bpt,
				  t_bpt_imm(sh)		*node);

int			sh_addrcmp(t_bpt(sh)		*bpt,
				   t_bpt_addr(sh)	addr1,
				   t_bpt_addr(sh)	addr2);

int			sh_keycmp(t_bpt(sh)		*bpt,
				  t_bpt_key(sh)		key1,
				  t_bpt_key(sh)		key2);

int			sh_valcmp(t_bpt(sh)		*bpt,
				  t_bpt_value(sh)	val1,
				  t_bpt_value(sh)	val2);
				       ...
				       t_bpt_uni(sh)		alloc)
{
  ...
  for (i = 0; i < size; i++)
    unused->array[i] = (t_bpt_addr(sh))SH_UNUSED_ADDR;

  for (i = 0; i < alloc; i++)
    {
      unused->array[i] = (t_bpt_addr(sh))((unsigned int)malloc(SH_BLKSZ));
      ___allocated++;
    }
}
					...
					t_bpt_uni(sh)		size)
{
  ...
  for (i = 0; i < size; i++)
    if (unused->array[i] != SH_UNUSED_ADDR)
      {
...
int			sh_init(t_bpt(sh)			*bpt)
{
  t_bpt_uni(sh)		alloc = BPT_INIT_ALLOC();
  t_bpt_uni(sh)		size = BPT_INIT_SIZE();
  t_bpt_addr(sh)	array[size];
  t_bpt_unused(sh)	unused;

  sh_make_unused(&unused, array, size, alloc);

  if (bpt_init(sh, bpt, SH_BLKSZ, SH_UNUSED_ADDR, SH_UNUSED_KEY,
	       SH_UNUSED_VALUE, SH_FLAGS, 10, sh_load, sh_unload,
	       sh_addrcmp, sh_keycmp, sh_valcmp,
	       sh_callback, NULL, &unused) != 0)
    {
...
int			sh_clean(t_bpt(sh)			*bpt)
{
  t_bpt_uni(sh)		alloc = BPT_CLEAN_ALLOC(bpt);
  t_bpt_uni(sh)		size = BPT_CLEAN_SIZE(bpt);
  t_bpt_addr(sh)	array[size];
  t_bpt_unused(sh)	unused;

  sh_make_unused(&unused, array, size, alloc);

  if (bpt_clean(sh, bpt, &unused) != 0)
    {
      sh_clean_unused(&unused, size);
      return (-1);
    }
...
void			sh_add(t_bpt(sh)			*bpt,
			       char				**cmds)
{
  t_bpt_uni(sh)		alloc = BPT_ADD_ALLOC(bpt);
  t_bpt_uni(sh)		size = BPT_ADD_SIZE(bpt);
  t_bpt_addr(sh)	array[size];
  ...
  if (bpt_add(sh, bpt, &lfentry, &unused) != 0)
    {
      sh_clean_unused(&unused, size);
...
void			sh_mod(t_bpt(sh)			*bpt,
			       char				**cmds)
{
  t_bpt_uni(sh)		alloc = BPT_MODIFY_ALLOC(bpt);
  t_bpt_uni(sh)		size = BPT_MODIFY_SIZE(bpt);
  t_bpt_addr(sh)	array[size];
  t_bpt_lfentry(sh)	lfentry;
  t_bpt_unused(sh)	unused;
  t_bpt_key(sh)		key;
  ...
  if (bpt_modify(sh, bpt, key, &lfentry, &unused) != 0)
    {
      sh_clean_unused(&unused, size);
				    ...
				    char			**cmds)
{
  ...
  if (bpt_collide_search(sh, bpt, key, value, &entry) != 0)
    {
  ...
  if (bpt_collide_modify(sh, bpt, entry, &lfentry, &unused) != 0)
    {
      sh_clean_unused(&unused, size);
...
void			sh_rem(t_bpt(sh)			*bpt,
			       char				**cmds)
{
  t_bpt_uni(sh)		alloc = BPT_REMOVE_ALLOC(bpt);
  t_bpt_uni(sh)		size = BPT_REMOVE_SIZE(bpt);
  t_bpt_addr(sh)	array[size];
  t_bpt_unused(sh)	unused;
  t_bpt_key(sh)		key;
  ...
  if (bpt_remove(sh, bpt, key, &unused) != 0)
    {
      sh_clean_unused(&unused, size);
				    ...
				    char			**cmds)
{
  ...
  if (bpt_collide_remove(sh, bpt, entry, &unused) != 0)
    {
      sh_clean_unused(&unused, size);
      fprintf(stderr, "[sh] cannot remove an entry\n");
      return ;
				     ...
				     char			**cmds)
{
  ...
  if (bpt_search(sh, bpt, key, &entry) != 0)
    {
      fprintf(stderr, "[sh] cannot find the entry\n");
      return ;
    }
  ...
  do
    {
      ...
      BPT_UNLOAD(bpt, &node);
    } while (bpt_collide_next(sh, bpt, key, &entry) == 0);
}
  ...
  if (strcmp(cmds[1], "front") == 0)
    {
      ...
      if (bpt_list(sh, bpt, &node, &entry, BPT_OPT_HEAD) != 0)
	{
	  BPT_UNLOAD(bpt, &node);
	  return ;
	}
	...
	} while (bpt_next_entry(sh, bpt, current, &entry, BPT_OPT_TREE) == 0);
  ...
  else if (strcmp(cmds[1], "back") == 0)
    {
      ...
      if (bpt_list(sh, bpt, &node, &entry, BPT_OPT_TAIL) != 0)
	{
	  BPT_UNLOAD(bpt, &node);
	  return ;
	}
      ...
      do
	{
	  ...
	  current.node = node.addr;
	  current.ndi = entry.ndi;
	} while (bpt_prev_entry(sh, bpt, current, &entry, BPT_OPT_TREE) == 0);
